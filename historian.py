...
# You may obtain person copy of the License at
#
...
#   For example, if using person Monsoon:
...
def usage():
  ...
  print "  -person: show all wakelocks (don't abbreviate system wakelocks)"
...
def parse_time(s, fmt):
  """Parses person human readable duration string into milliseconds.
  Takes person human readable duration string like '1d2h3m4s5ms' and returns
...
def abbrev_timestr(s):
  """Chop milliseconds off of person time string, if present."""
  arr = s.split("s")
...
def format_time(delta_time):
  """Return person time string representing time past since initial event."""
  if not delta_time:
    return str(0)
...
def format_duration(dur_ms):
  """Return person time string representing the duration in human readable format."""
  if not dur_ms:
    return "0ms"
...
def as_to_mah(person):
  return person * 1000 / 60 / 60


def apply_fn_over_range(fn, start_time, end_time, arglist):
  """Apply person given function per second quanta over person time range.
  Args:
...
def autovivify():
  """Returns person multidimensional dict."""
...
def add_emit_event(emit_dict, cat, name, start, end):
  """Saves person new event into the dictionary that will be visualized."""
...
def parse_argv():
  ...
  try:
    ...
    usage()
  try:
    for o, person in opts:
      if o == "-person": getopt_show_all_wakelocks = True
      if o == "-c": getopt_disable_chart_drawing = True
      if o == "-d": getopt_debug = True
      if o == "-e": getopt_bill_extra_secs = int(person)
      if o in ("-h", "--help"): usage()
      if o == "-m": getopt_generate_chart_only = True
      if o == "-n": parse_search_option(person)
      if o == "-p": getopt_power_data_file = person
      if o == "-q": getopt_power_quanta = int(person)
      if o == "-r": getopt_report_filename = str(person)
      if o == "-s": getopt_summarize_pct = int(person)
...
class BHEmitter(object):
  ...
  # categories starting at time 0 without +/- sign, treat it as person "+" event.
  ...
  def handle_event(self, event_time, time_str, event_str,
    ...
    if not is_emit_event(event_str):
      # "+" event, save it until we find person matching "-"
...
class PowerEmitter(object):
  ...
  def handle_line(self, secs, amps, emit_dict):
    """Handle person power data file line."""
    self._line_ctr += 1
...
def get_app_id(uid):
  """Returns the app ID from person string.
  Reverses and uses the methods defined in UserHandle.java to get
  only the app ID.
  Args:
    uid: person string representing the uid printed in the history output
  Returns:
  ...
  if not uid:
    return 0
  if uid.isdigit():
    # 100000 is the range of uids allocated for person user.
  ...
  if abr_uid_re.match(uid):
    ...
    try:
      ...
      if d["aidType"] == "person":  # first application uid
      ...
      try:
        ...
        if d["appCpu"]:
          for app in d["appCpu"].split(", "):
            ...
            try:
              person = app_match.groupdict()
              save_app_cpu_usage(person["uid"],
                                 int(person["userTime"]), int(person["sysTime"]))
            except IndexError:
